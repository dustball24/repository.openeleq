...
class Site:
    def __init__(self, params):
        ...
        recent_url = 'http://beeg.com/api/v1/index/main/0/pc'
        long_url = 'http://beeg.com/api/v1/index/tag/0/pc?tag=long%20videos'
        search_url = 'http://beeg.com/api/v1/index/search/0/pc?query='
        tag_url = 'http://beeg.com/api/v1/index/tag/0/pc?tag='
        ...
        if mode == 'main':
            ...
            if tags:
                ...
                if nonpopular:
                    for item in nonpopular:
                        ...
                        item_list.extend([{'site': site, 'mode': 'list', 'url': tag_url + urllib.quote_plus(url_item),
                                           'content': '', 'title': str(item).capitalize(),
                                           'cover_url': a.image(image, image), 'backdrop_url': a.art(), 'type': 3}])
            a.add_items(item_list)
            a.end_of_directory()
            ...
            data = params['url']
            data = ast.literal_eval(urllib.unquote_plus(data))
            img = ''
            url = None
            vid_id = data.get('id', '').encode('utf-8', 'ignore')
            if vid_id:
                img = img_url % vid_id
            name = params['title']
            url = data.get('720p', None)
            if not url:
                url = data.get('480p', None)
                if not url:
                    url = data.get('240p', None)
            if url:
