...
class Site:
    def __init__(self, params):
        ...
        home_url = 'http://www.watchxxxfree.com/'
        search_url = home_url + '?s='
        false_positives = ['http://www.watchxxxfree.com/watch-full-movies-hd/', 'http://www.watchxxxfree.com',
                           'http://www.watchxxxfree.com/category/movies/',
                           'http://www.watchxxxfree.com/category/ategorized222/',
                           'http://www.watchxxxfree.com/watch-full-movies-hd/']

        if mode == 'main':
            item_list = [{'site': site, 'mode': 'list', 'title': a.language(30003), 'content': '',
                          'url': home_url + '?filtre=date&cat=0', 'cover_url': a.image('recent.png', image),
                         ...
                         {'site': site, 'mode': 'categories', 'title': a.language(30005), 'content': '',
                          ...
                          'backdrop_url': a.art(), 'type': 3},
                         {'site': site, 'mode': 'topstars', 'title': a.language(30015), 'content': '',
                          'url': home_url + 'top-pornstars/', 'cover_url': a.image('toppornstars.png', image),
                          'backdrop_url': a.art(), 'type': 3},
            ...
            html = a.get_page(params['url'])
            soup = BeautifulSoup(html, parseOnlyThese=SoupStrainer('ul', {'class': 'wp-tag-cloud'}))
            item_list = []
            if soup:
                for item in soup.findAll('li'):
                    if item:
                        if item.a.get('href') not in false_positives:
                            try:
                                vidcount = item.a.get('title')
                                vidcount = re.sub('\stopic[s]*', '', vidcount)
                            except:
                                vidcount = '0'
                            if vidcount and vidcount != '0':
                                title = item.a.string.encode('UTF-8') + ' (%s)' % vidcount
                                item_list.extend([{'site': site, 'mode': 'list', 'url': item.a.get('href'),
                                                   'content': '', 'title': title,
            ...
            if params.get('content', '') == 'search':
                ...
                if item:
                    ...
                    exit(1)
            html = a.get_page(params['url'])
            soup = BeautifulSoup(html, parseOnlyThese=SoupStrainer('ul', {'class': 'listing-videos listing-tube'}))
            item_list = []
            params['mode'] = 'play'
            ...
            if soup:
                ...
                for item in soup.findAll('li', {'class': 'border-radius-5 box-shadow'}):
                    if item:
                        if item.a.get('href') not in false_positives:
                            _dict = xbmcdict.copy()
                            if 'full movie' not in _dict['title'].lower() and 'xtheatre' not in _dict['title'].lower():
                                _dict['duration'] = '1500'
